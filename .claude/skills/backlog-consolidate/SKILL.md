...
8. **Present findings** — Show the user the full list, grouped by severity, with evidence:
   ...
   HIGH:
   2. SCOPE OVERLAP: BEAN-073 and BEAN-076 both modify `src/`
...
- **Scope overlap**: Extract file paths (`src/`), module names (`generator`, `compiler`), service names (`Scaffold Service`), and feature areas (`wizard pages`, `CLI`) from In Scope sections. If two beans reference the same items, evaluate whether the overlap is incidental (both mention `_index.md` because they add beans) or substantive (both modify the same service logic).
