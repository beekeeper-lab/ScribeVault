...
in the ScribeVault pipeline (Select --> Compose --> Compile --> **Scaffold** --> Seed --> Export).

## Trigger

- Invoked by the `/scaffold-project` slash command.
- Called programmatically by `src/` during pipeline execution.
- Runs automatically as part of the project generation pipeline.
...
- `src/` -- CompositionSpec data contract
