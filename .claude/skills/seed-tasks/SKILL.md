...
personas execute. It runs during the Seed phase of the ScribeVault pipeline
(Select --> Compose --> Compile --> Scaffold --> **Seed** --> Export).

## Trigger

- Invoked by the `/seed-tasks` slash command.
- Called programmatically by `src/` at project kickoff or cycle start.
...
- `src/` -- reference implementation of the seed logic
- `src/` -- CompositionSpec model for parsing team composition
