...
.claude/              # Claude Code config (submodule + local)
  ...
  commands/           # Assembled symlinks (generated by claude-assemble.sh)
  skills/             # Assembled symlinks (generated by claude-assemble.sh)
  settings.json -> shared/settings.json
scripts/              # Helper scripts (claude-sync, claude-publish, claude-assemble)
```
...
Claude Code discovers commands at `.claude/commands/` and skills at `.claude/skills/` — it does **not** scan nested subdirectories like `shared/` or `local/` automatically. The assembly script (`scripts/claude-assemble.sh`) bridges this by creating symlinks from the discovery directories into both `shared/` and `local/` sources.

```bash
# Pull latest kit + sync submodule + assemble discovery symlinks
./scripts/claude-sync.sh

# Reassemble after adding new local skills/commands
./scripts/claude-assemble.sh
