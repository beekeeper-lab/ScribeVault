...
from ai.prompt_templates import PromptTemplate, PromptTemplateManager, BUILTIN_TEMPLATES
...
class TestPromptTemplate(unittest.TestCase):
    ...
    def test_create_template(self):
        ...
        t = PromptTemplate(template_id="test-1", name="Test Template", prompt_text="Summarize this text.")
    ...
    def test_to_dict(self):
        ...
        t = PromptTemplate(
            template_id="test-1", name="Test", prompt_text="prompt", is_builtin=True, created_at="2026-01-01T00:00:00"
        )
        d = t.to_dict()
    ...
    def test_roundtrip(self):
        ...
        original = PromptTemplate(template_id="rt-1", name="Roundtrip", prompt_text="Extract key points from this.")
...
class TestBuiltinTemplates(unittest.TestCase):
    ...
    def test_builtin_names(self):
        ...
        expected = {"Action Items", "Key Decisions", "Brief Summary", "Detailed Notes", "Meeting Minutes"}
    ...
    def test_builtins_have_prompt_text(self):
        ...
        for t in BUILTIN_TEMPLATES:
            self.assertTrue(len(t.prompt_text.strip()) > 0, f"{t.name} has empty prompt_text")
...
class TestPromptTemplateManager(unittest.TestCase):
    ...
    def test_save_custom_template(self):
        ...
        template = self.manager.save_custom_template("My Template", "Summarize briefly.")
    ...
    def test_special_characters_in_template_name(self):
        ...
        t = self.manager.save_custom_template("Template with 'quotes' & <special> chars!", "Some prompt")
        retrieved = self.manager.get_template(t.template_id)
        self.assertEqual(retrieved.name, "Template with 'quotes' & <special> chars!")
