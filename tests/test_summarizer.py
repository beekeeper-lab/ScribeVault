...
class TestSummarizeWithPrompt(unittest.TestCase):
    ...
    def test_summarize_with_custom_prompt(self):
        ...
        self.mock_client.chat.completions.create.return_value = _make_mock_response("Action item 1\nAction item 2")

        result = self.service.summarize_with_prompt(
            "Meeting transcript...", "Extract action items from this transcript."
        )
        ...
        self.assertEqual(messages[0]["content"], "Extract action items from this transcript.")
    ...
    def test_summarize_with_prompt_uses_higher_max_tokens(self):
        ...
        self.mock_client.chat.completions.create.return_value = _make_mock_response("result")
...
class TestStyleParameterFix(unittest.TestCase):
    ...
    def test_meeting_with_default_style_uses_structured(self):
        ...
        self.mock_client.chat.completions.create.return_value = _make_mock_response("Structured meeting summary")

        result = self.service.generate_summary_with_markdown(
            {"transcription": "Meeting text...", "category": "meeting"}, style="concise"
        )

        self.assertEqual(result["summary"], "Structured meeting summary")
    ...
    def test_meeting_with_explicit_style_uses_style(self):
        """Meetings with explicit non-default style respect the style parameter."""
        self.mock_client.chat.completions.create.return_value = _make_mock_response("Bullet point summary")

        result = self.service.generate_summary_with_markdown(
            {"transcription": "Meeting text...", "category": "meeting"}, style="bullet_points"
        )
    ...
    def test_meeting_with_detailed_style(self):
        ...
        self.mock_client.chat.completions.create.return_value = _make_mock_response("Detailed summary")

        result = self.service.generate_summary_with_markdown(
            {"transcription": "Meeting text...", "category": "meeting"}, style="detailed"
        )
    ...
    def test_custom_prompt_overrides_all(self):
        ...
        self.mock_client.chat.completions.create.return_value = _make_mock_response("Custom result")
        ...
        call_args = self.mock_client.chat.completions.create.call_args
        system_msg = call_args[1]["messages"][0]["content"]
        self.assertEqual(system_msg, "List all mentioned technologies.")

    def test_non_meeting_with_concise_style(self):
        """Non-meeting recordings with concise style use concise prompt."""
        self.mock_client.chat.completions.create.return_value = _make_mock_response("Concise note")

        result = self.service.generate_summary_with_markdown(
            {"transcription": "Note text...", "category": "note"}, style="concise"
        )
    ...
    def test_call_category_default_uses_structured(self):
        ...
        self.mock_client.chat.completions.create.return_value = _make_mock_response("Call structured")

        self.service.generate_summary_with_markdown(
            {"transcription": "Call text...", "category": "call"}, style="concise"
        )
    ...
    def test_interview_with_bullet_points_style(self):
        """Interview with explicit bullet_points style respects the style."""
        self.mock_client.chat.completions.create.return_value = _make_mock_response("Interview bullets")

        self.service.generate_summary_with_markdown(
            {"transcription": "Interview text...", "category": "interview"}, style="bullet_points"
        )
...
class TestSummaryHistoryStorage(unittest.TestCase):
    ...
    def test_add_summary_creates_history(self):
        ...
        rec_id = self.vault_manager.add_recording(filename="test.wav", transcription="Some text")

        self.vault_manager.add_summary(rec_id, "Summary 1", template_name="Brief Summary")
        ...
        self.assertEqual(rec["summary_history"][0]["template_name"], "Brief Summary")
    ...
    def test_add_multiple_summaries(self):
        ...
        self.vault_manager.add_summary(rec_id, "Second summary", template_name="Action Items")
        self.vault_manager.add_summary(rec_id, "Third summary", template_name="Custom")
        ...
        self.assertEqual(rec["summary_history"][1]["content"], "Second summary")
    ...
    def test_summary_history_preserves_prompt(self):
        ...
        rec_id = self.vault_manager.add_recording(filename="test.wav")
        self.vault_manager.add_summary(rec_id, "Summary", template_name="Custom", prompt_used="Extract action items")
